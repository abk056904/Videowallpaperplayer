...
| HW decode — D3D11VA fix + CPU optimization | ✅ | 2026-08-20 | Fixed 3 bugs preventing HW decode; D3D11VA GPU-to-GPU shared handles (zero CPU↔GPU copies); CPU optimizations (cached texture, bulk NV12, reusable swFrame). Measured: CPU 190%→59%, RAM 424→216 MB, 60 fps, 0 drops |
...
| HW decode + D3D11VA zero-copy + CPU opt | ✅ | 2026-08-20 | Fixed DecoderFactory (MF SW fallback too early), FFmpegDecoder (wrong decoder name), device-sharing crash. D3D11VA active via shared handles + deferred context (GPU-to-GPU copy, zero CPU↔GPU). CPU optimizations: cached shared texture, bulk NV12 upload, reusable swFrame. Measured: CPU 59.2% of 1 core, RAM 216 MB, 60 fps decode+present, 0 drops, decode latency 19–23 ms |
...
- **GPU/VRAM measured (2026-08-18)**: AMD iGPU dedicated VRAM 32 MB avg (range 29.6–34.9 MB, flat — no growth), shared 43 MB; AMD 3D engine 0.0% avg, COPY 0.0% avg (render 0.3 ms / 16.7 ms frame = 1.8% theoretical). NVIDIA dGPU 32 MiB VRAM (system composition only). All via Windows GPU Process Memory + Engine counters + nvidia-smi, 15 samples @ 3 s.
- **NOT MEASURED** — 4K/AV1/HDR stress rows (no HW MFT, M5), power/hot-plug scenarios (declined — disruptive, single display).
...
- [x] **Frame-queue depth fix (2026-08-18, user request "reduce the ~3/s stale-frame drops")** — the earlier experiment only tried 3 → 6 (too deep). Measuring down settled it: `frameQueue` **1 → 0.00 drops/s**, presented = decoded = 60.1 fps, latency 65 → ~16 ms, RAM −18 MB, decode-bound behavior identical at any depth. **Mechanism:** capacity 1 makes the decode worker consumer-paced (blocks on push when full) — the queue can never hold two due frames at a consumer wake, so the freshness counter cannot increment by construction. **Default changed 3 → 1** (config + UiContract + VideoPlayer); range 1–16 kept (documented drops↔buffering tradeoff). Verified live: 35 steady-state samples, 0 drops; 172/172 tests Debug + Release, 0 warnings.
- [x] **UI fixes (2026-08-18, user report "parts of ui are broken / show video names")** — two real bugs fixed + live-verified:
  - **Playlist Name column was EMPTY**: `PlaylistsPanel::rebuildItems` inserted the full path into column 0 then overwrote it with the row number — column 1 (Name) was never populated. Now shows the video **file name** (same pattern as the Monitors source combo); sub-cell copies truncate safely (wcsncpy_s, never fail-empty on long filenames). **Verified live** via a safe cross-process ListView read (VirtualAllocEx/ReadProcessMemory in the target — the naive LVM_GETITEMTEXT from another process CRASHES the app, see below): rows now read `1 | Arlecchino-….mp4 | - | - | ✓` etc.
  - **Monitors-panel Preview was broken by the NV12 optimization (B1)**: `grabFrameSnapshot` returned "no preview on the NV12 path" — the software path is NV12 now, so the button error-boxed. Now the readback **converts NV12→BGRA on the CPU** (same BT.709 limited→full-range math as the GPU shader; user-initiated, one frame — fine). Only the transient hardware path still reports no preview (documented). **Verified live**: Preview click sets the preview static's bitmap (STM_GETIMAGE non-zero), no "preview grab failed" in the log, app healthy.
  - **Investigation note**: the app appeared to "crash on startup" during probing — actually my own cross-process LVM_GETITEMTEXT probe crashed it (listview dereferenced a foreign pointer → access violation in COMCTL32, WER Application Error). Not an app bug. Safe probes kept: `build/release/ui_verify_safe.ps1` + `preview_verify.ps1`.
