...
- **2026-08-17 (M8 review)**: `associateAdapters` had a **first-match bug** — it broke only the inner (output) loop, so a later adapter's overlapping output could overwrite a correct association (real on virtual-display/surround layouts). Fixed with a `matched` flag (first match wins); **1 regression test** (overlapping output rects → adapter 0 + LUID stick) → **123/123**, both configs, 0 warnings.
