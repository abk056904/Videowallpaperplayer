...
>
> On Windows systems with hybrid/integrated + discrete GPUs, the OS can choose the integrated GPU by default. To force the app to use a discrete GPU (for example NVIDIA or AMD) and enable vendor hardware decode/acceleration, add the application's executable to Settings → System → Display → Graphics settings (Browse → select the .exe), open Options for the app, choose "High performance" and select the discrete GPU. Restart the app after changing this setting.
