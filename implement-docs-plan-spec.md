...
- **M10 (config cross-validation + live reactivity):** validate threshold **pairs** — pause threshold must be ≥ its resume threshold (cpu/gpu/memory) at load **and** on every `CONFIG_SET`; violations are clamped (resume pulled toward pause) and logged. Add a **config revision counter** (bumped on every accepted change) so `ResourceGovernor` reacts to live threshold/delay/mode changes without polling `ConfigurationManager` (today config is loaded once at startup).
- **M11 (config write-batching):** UI-triggered `CONFIG_SET` changes are persisted with a **debounced dirty-flag save** (≈1–2 s after the last change, plus save on shutdown) instead of writing per click — today config only saves at shutdown, so UI edits would be lost on crash and per-click writes would hammer the disk.
...
| M10 | `ResourceGovernor*`, `PausePolicy*`, `SystemStateMonitor*` | Governor transition table (source spec doc 3 §95) passes; battery/long-pause release; resume; **threshold-pair cross-validation (pause ≥ resume)** + config revision counter | Auto-pause, Efficiency, Suspension | Pure-logic unit tests; **lock/unlock NOT MEASURED (declined)**; **display-off testable** via `powercfg` monitor-timeout (non-disruptive); **battery testable** by unplugging AC (user-assisted); suspend/resume logic code-reviewed (hibernation too disruptive); cross-validation + revision-counter unit tests |
| M11 | `Win32UI*`, panels, `TrayController*`, `LibraryManager*` | Panels functional; tray works with UI closed; no RAM growth on open/close; incremental scan; **config write-batching (debounced dirty save)** | UI | Manual + library-scan unit tests; UI-open/close leak cycle; save-batching unit tests (debounce + crash-safe: last state persisted) |
